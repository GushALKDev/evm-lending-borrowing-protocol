...
- [x] **0.5** Setup linters (Solhint, `forge fmt`)
...
> **8.10 note:** added after a review found that no test ran `absorb` or `buyCollateral` through the
> real oracle or with a non-zero `msg.value`. The fork suite cannot reach them (a cached VAA cannot
> move the price) and every other layer prices through `MockPriceOracle`, which charges no fee. That
> left `_pushBuyPrices` and the liquidation-side refund sweep unexercised on exactly the path where
> Phase 5 found the missing `receive()`. Mutation-checked: dropping `_refundExcessValue()` from both
...
| 2026-07-24 | Phase 8 end-to-end integration (8.6): `FullLifecycleTest` drives supply, borrow, warp, repay, absorb, and buyCollateral in one sequence on a production `LendingMarket` priced by `MockPriceOracle`, asserting the tie-in invariants at every hand-off; `DeployScriptTest` rehearses `Deploy.s.sol` against deployed dependencies exported into its env vars. Testing docs synced with the real Phase 8 state. Phase 9 (Future Work) excluded from the progress total, which is recomputed over phases 0-8 only. 265 total green |
| 2026-07-24 | Phase 8 quote rounding (8.5): `QuoteRoundingTest` pins `quoteCollateral` against its exact floored quotient, so a flipped floor fails by one wei where the absorb round trip would survive it; storefront and liquidation factors fuzzed within the INV-13 bound, plus quote monotonicity in base paid and an ask never above spot. Conversion, rate, and quote math now all carry per-site directed-rounding assertions. 263 total green |
