...
2. **Repayments are sacred; exits are paused only on evidence.** Repay and `accrue` are the mechanisms by which users and the protocol heal, and no flag blocks them: `PAUSE_SUPPLY` stops new exposure only, so a repay of an account in debt (up to its debt, by the account or a third party through `supplyTo`) and a collateral top-up of an account in debt still run, and a supply pause never leaves a borrower absorbable yet unable to defend the position. Borrowing has its own flag: `PAUSE_BORROW` stops a base withdrawal that would open or increase debt and any collateral withdrawal from an account in debt, and nothing else. The exits that *can* be paused are `withdraw` as a whole (`PAUSE_WITHDRAW`, which stops supplier exits and every collateral withdrawal along with borrowing) and lmUSDC transfers (`PAUSE_TRANSFER`); each strands suppliers while set, so it is reserved for an accounting bug, where the withdrawal amounts themselves are suspect.
...
Scenario playbook. Each row starts from no flags and adds only the flags with a concrete reason. Supplier exits (`withdraw` of base up to the positive balance), repayment, and collateral top-ups of accounts in debt stay open unless the row says why they close. Repayment and top-ups are never closed by any flag; only `PAUSE_WITHDRAW` closes supplier exits.

| Incident | Flags to set | Stays open | Reason for each flag |
| :------- | :----------- | :--------- | :------------------- |
| Collateral token exploit (unbacked mint or bridge compromise of collateral X), X's oracle price still valid | `BORROW + SUPPLY` | Supplier exits, repay, top-ups, transfers, absorb, buyCollateral | `BORROW`: stops borrowing against freshly minted X and stops a debtor from withdrawing sound collateral while X keeps the position healthy on paper. `SUPPLY`: new suppliers would join a pool carrying a pending loss on debt backed by X. `ABSORB` stays open: at a valid price an absorb converts the X-backed debt into inventory now instead of into bad debt after X reprices. `BUY` stays open: it is how the protocol sells that inventory for base; a sale pays base in and sends X out, so it cannot be used to push X into the market. `WITHDRAW` is not needed, since `BORROW` already stops every withdrawal that adds risk. Residual: the top-up exception still lets a debtor post X to delay its own absorb (see below) |
| Oracle passing its checks but confirmed wrong | `BORROW + ABSORB + BUY + SUPPLY` | Supplier exits, repay, top-ups, transfers | `BORROW`: new debt and collateral withdrawals are priced by the wrong feed. `ABSORB`: eligibility and the seize credit use the wrong price, so an absorb either takes a healthy account or overpays for collateral; the one case where waiting is better than absorbing. `BUY`: the storefront quote uses the wrong price. `SUPPLY`: new suppliers would join while liquidations are off and bad debt can build. Exits and repay read no price, so nothing requires closing them |
| Oracle outage (feeds failing their checks) | none | Everything that reads no price | The failing checks already stop borrowing, collateral withdrawal while in debt, absorb, and buyCollateral for the affected accounts ([S2](#s2-oracle-outage-during-a-drawdown)); a flag would add nothing and would outlive the outage |
| Suspected accounting bug | `WITHDRAW + TRANSFER + SUPPLY + BUY + BORROW` | Repay, top-ups, absorb | `WITHDRAW`: withdrawal amounts come from the suspect principals and indexes, so exits would pay out wrong values first come first served; this is the one row that closes supplier exits. `TRANSFER`: moves possibly wrong balances to third parties and changes the principals that must be reconciled. `SUPPLY`: new deposits into a ledger under suspicion. `BUY`: gated on `getReserves()`, which is derived from the suspect totals. `BORROW`: redundant while `WITHDRAW` is set, but it keeps borrowing closed if the owner reopens exits first. `ABSORB` stays open unless the bug is in debt, index, or health math, in which case add it, since eligibility would then be wrong |
| Market migration (end of life) | `SUPPLY + BORROW` | Supplier exits, repay, top-ups, debt-free collateral withdrawals, transfers, absorb, buyCollateral | `SUPPLY`: no new deposits into a market being retired. `BORROW`: no new debt; existing debt closes by repayment or absorb. Transfers stay open: nothing unsafe depends on them, and closing them would strand integrations that hold lmUSDC |
| Owner key incident | none | Everything | No flag restrains the owner: it can clear any flag, and `withdrawReserves` is not pausable. The response is to move ownership with the two-step transfer while the key is still controlled; if it is lost, migrate |

**Residual: collateral top-ups and an exploited asset.** The `PAUSE_SUPPLY` exception lets any account in debt post any listed collateral. During a collateral exploit, a debtor can post freshly minted X, valued at its still-valid price, to stay clear of absorb. It cannot borrow more (`PAUSE_BORROW`) or take its sound collateral out, so its debt does not grow, but the absorb that would have seized its collateral while X still had a price is delayed. The flags are global, so there is no way to refuse top-ups of one asset while keeping them for the others.
...
**Handler design.** One handler contract wrapping every public mutating function with bounded random inputs, driving a cast of actors (3 suppliers, 3 borrowers, 1 liquidator, owner, guardian) plus a `warp` action (time jumps up to 30 days), a `movePrice` action (oracle mock steps within and beyond confidence bounds), a `togglePause` action (random `PAUSE_*` flag sets by the owner, so every invariant also runs under partial pauses), a `guardianPause` action (the guardian adding one flag at a time, `PAUSE_BORROW` included), and a `repayWhileSupplyPaused` action (repays of a random debtor, by the debtor or a third party, with `PAUSE_SUPPLY` set). Ghost variables track every base inflow/outflow (for INV-5) and the reserves around each call (for INV-4); per-action properties are latched by the handler and asserted globally.

**Asserted properties per run:** INV-1 through INV-11, with INV-3 checked at the live indexes (the stateless fuzz covers the full index domain) and INV-9/INV-10 as per-action latches; INV-14 at the live utilization; absorb-only-when-liquidatable (and its converse, that the view and the check never disagree); a debtor with tokens and approval can always repay while `PAUSE_SUPPLY` is set; while `PAUSE_BORROW` is set no account's debt principal grows and no indebted account's collateral falls outside absorb, checked on every actor around every action; and `getReserves()` deltas matching the per-operation monotonicity table in [Guide 2, Section 6](./02-mathematics.md#6-interest-split-and-reserve-growth) exactly, with the pure accrue at a 1-wei tolerance. INV-12 and INV-13 are constructor-enforced and immutable, so they are tested at construction, not per step.
