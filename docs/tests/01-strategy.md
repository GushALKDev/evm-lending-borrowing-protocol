...
| [`MockPriceOracle.sol`](../../test/mocks/MockPriceOracle.sol) | Per-asset `(price18, conf18)` set via `setPrice`. Wired since Phase 3; exercised from Phase 4 on, when health checks start reading prices. `unsetPrice` clears a feed so both read paths revert, which is how the repay path is tested to need no oracle at all. Retained for the market suites that don't need the real validation pipeline. |
| [`MockPyth.sol`](../../lib/pyth-sdk-solidity/MockPyth.sol) (SDK) | The Pyth SDK's own mock, used from Phase 5 to test `PythChainlinkOracle`. Real `createPriceFeedUpdateData` and fee-charging `updatePriceFeeds`, so the fee/refund path is exercised, not stubbed. Stores only a strictly newer `publishTime` — a same-timestamp re-push is a no-op. |
| [`MockChainlinkFeed.sol`](../../test/mocks/MockChainlinkFeed.sol) | Settable `AggregatorV3` stand-in for the deviation anchor: `setAnswer` drives the answer and its `updatedAt`, and the constructor sets `decimals()`, so the anchor price, its heartbeat staleness, and non-8-decimal normalization can all be driven on demand. |
| [`ForcedEth.sol`](../../test/mocks/ForcedEth.sol) | `RejectingCaller`, a contract with no `receive` or fallback that forwards arbitrary calls with value, and `ForceSender`, which pushes ETH into a contract with `selfdestruct`. Used by the forced-ETH refund tests in the integration and fork suites. |
