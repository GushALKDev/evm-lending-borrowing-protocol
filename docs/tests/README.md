...
**Status:** Updated at the close of every roadmap phase (current: Phase 3)
...
**126 tests, all green** (Phase 3 close).
...
| `src/LendingMarket.sol`     | 97.82% (224/229) | 95.09% (271/285) | 78.85% (41/52) | 97.78% (44/45) |
| **Total**                   | **97.97%**       | **95.48%**       | **80.36%**     | **97.92%**     |

The five uncovered lines in `LendingMarket.sol` are the borrow/repay health hook, a `NotImplementedYet` stub until Phase 4. Branch coverage is the metric still below target, and the uncovered branches are predominantly those same Phase 4-7 stubs; the >95% gate applies at Phase 8. Details in [Gaps & Roadmap](./07-gaps-and-roadmap.md).
...
| INV-9     | No action ends undercollateralized                | ⏳ Phase 4 (needs the real health check)                                                            |
| INV-10    | `minBorrow` dust guard                            | ⏳ Phase 4                                                                                          |
